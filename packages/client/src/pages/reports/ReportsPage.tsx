        ...
        <div className="card p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="rounded-xl bg-surface-container-low p-4">
              <p className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Sinking fund billed</p>
              <p className="mt-1 text-lg font-semibold text-on-surface">{formatCurrency(data?.reserveFunds?.sinkingFundBilled || 0)}</p>
              <p className="mt-1 text-xs text-on-surface-variant">Collected {formatCurrency(data?.reserveFunds?.sinkingFundCollected || 0)}</p>
            </div>
            <div className="rounded-xl bg-surface-container-low p-4">
              <p className="text-xs font-bold uppercase tracking-widest text-on-surface-variant">Repair fund billed</p>
              <p className="mt-1 text-lg font-semibold text-on-surface">{formatCurrency(data?.reserveFunds?.repairFundBilled || 0)}</p>
              <p className="mt-1 text-xs text-on-surface-variant">Collected {formatCurrency(data?.reserveFunds?.repairFundCollected || 0)}</p>
            </div>
            <div className="rounded-xl border border-outline-variant/15 bg-surface-container-low px-4 py-3 sm:col-span-2">
