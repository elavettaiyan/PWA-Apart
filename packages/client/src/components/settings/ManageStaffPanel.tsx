...
export default function ManageStaffPanel({ embedded = false }: { embedded?: boolean }) {
  ...
  return (
    <div className="space-y-5">
      ...
      {staff.length === 0 ? (
        <div className={cn(embedded ? 'rounded-2xl border border-outline-variant/15 bg-surface-container-low p-6 text-center' : 'card p-6 text-center')}>
...
function StaffCard({ member, embedded }: { member: StaffMember; embedded: boolean }) {
  ...
  return (
    <div className={cn(embedded ? 'rounded-2xl border border-outline-variant/15 bg-surface-container-low p-5' : 'card p-5', !member.isActive ? 'opacity-60' : '')}>
      <div className="flex items-start justify-between mb-3 gap-3">
        ...
        <button
          className={cn(
            ...
            member.isActive ? 'bg-emerald-50 text-emerald-900' : 'bg-rose-50 text-rose-900',
          )}
